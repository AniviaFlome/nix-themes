...
in
{
  ...
  imports = map (
    # TODO: remove – this renames legacy catppuccin options that do not exist in themes
    option: lib.mkRenamedOptionModule (ctpVscOptionPath ++ [ option ]) (newOptionPath ++ [ option ])
  ) renamedOptions;

  options.themes.vscode.profiles = lib.mkOption {
    type = types.attrsOf (
      types.submodule (
        { config, ... }:
        {
          options =
            ...
            // {
              ...
              icons = themesLib.mkThemeOption { name = "vscode-icons"; };
            };
        }
      )
    );
  ...
  config.programs.vscode.profiles = lib.mapAttrs (
    ...
    lib.mkIf profile.enable {
      # extensions = [
      #   (sources.vscode.override { catppuccinOptions = profile.settings; }) # TODO: generate this content inline from p.* palette colors (sources not available in nix-themes)
      # ]
      # ++ lib.optional (profile.icons.enable) sources.vscode-icons; # TODO: generate this content inline from p.* palette colors (sources not available in nix-themes)

      userSettings = lib.mkMerge [
        {
          # TODO: mkFlavorName not in themesLib – use config.themes.variant instead
          "workbench.colorTheme" = "Catppuccin " + (themesLib.mkFlavorName profile.flavor);
